"""Step 03c: Run the STAMP baseline on per-celltype Human Brain MERFISH AnnData.

Examples::

    python 03c_run_stamp.py --celltypes micro opc --device cuda:0
...
# MIN_CUTOFF_SCHEDULE = (0.15, 0.20, 0.25, 0.30, 0.40, 0.50)
TOP_K_GENES = 15
...
def run_stamp(
    ...
    *, n_topics: int, device: str, max_epochs: int, min_epochs: int, seed: int,
    ...
    base.obs["age"] = pd.to_numeric(base.obs["age"], errors="coerce").astype("float32")
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Run the STAMP baseline on per-celltype Human Brain MERFISH AnnData.",
