#!/usr/bin/env Rscript
# ----------------------------------------------------------------------------
# Step 03b: Multi-sample SpatialPCA on Human Brain MERFISH AnnData.
#
# Mirrors SpatialPCA_Multiple_Sample (Shang & Zhou, v1.2.0). Deviations:
#   * Skip SPARK SVG selection (irrelevant for the 285-gene MERFISH panel) ->
...
#
# The grouping column is `id_region` (donor_id + region_id) -- the human
# analogue of the mouse pipeline's `mouse_id`. Use this to separate tissue
# slices; age is not used to define samples here.
#
# Usage:
#   Rscript 03b_run_spatialpca.R --all
#   Rscript 03b_run_spatialpca.R --celltypes micro opc
...
# ════════════════════════════════════════════════════════════════════════════
# Single-celltype runner: AnnData -> per-sample lists -> fit -> outputs.
# ════════════════════════════════════════════════════════════════════════════
...
run_one <- function(input_h5ad, out_dir, k, group_col, bandwidth, seed) {
    ...
    # anndataR returns adata$X as dgRMatrix (CSR). Matrix >= 1.5 has no direct
    # dgRMatrix -> dgCMatrix coercion, and t(dgRMatrix) preserves the class, so
    # t(t(x)) is a no-op classwise. Go through CsparseMatrix instead.
    expr <- as(adata$X, "CsparseMatrix")              # cells x genes (dgCMatrix)
    rownames(expr) <- obs_names; colnames(expr) <- var_names
    count_all <- as(Matrix::t(expr), "CsparseMatrix") # genes x cells (dgCMatrix)
